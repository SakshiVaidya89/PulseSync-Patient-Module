...
export default function Navigation({ currentPage, onNavigate }: NavigationProps) {
  ...
  const navItems = [
    ...
    { id: "search", label: "Find Hospitals", icon: "" },
        ...
        <div className="flex justify-between items-center h-[80px]">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="w-10 h-10 rounded-full bg-gradient-to-br from-sky-500 to-cyan-500 flex items-center justify-center text-white font-bold hover:scale-110 transition-transform shadow-md relative"
              title="View Profile"
            >
              👤
              {!localStorage.getItem("userProfileComplete") && (
                <span className="w-3 h-3 bg-red-500 rounded-full absolute top-0 right-0 border-2 border-white"></span>
              )}
            </button>
